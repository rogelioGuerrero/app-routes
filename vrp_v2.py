...
async def vrp_v2(request: VRPSkillsRequest):
    ...
    # Callback para distancia
    def distance_callback(from_index, to_index):
        from_node = manager.IndexToNode(from_index)
        to_node = manager.IndexToNode(to_index)
        return int(distance_matrix[from_node][to_node])
        
    # Callback para tiempo (incluye tiempo de servicio)
    def time_callback(from_index, to_index):
        from_node = manager.IndexToNode(from_index)
        to_node = manager.IndexToNode(to_index)
        service_time = service_times[from_node] if hasattr(request, 'service_times') else 0
        return int(time_matrix[from_node][to_node] + service_time)
    
    # Registrar callbacks
    transit_callback_index = routing.RegisterTransitCallback(distance_callback)
    time_callback_index = routing.RegisterTransitCallback(time_callback)
    
    # Configurar objetivo de optimización
    ...
    routing.AddDimension(
        time_callback_index,  # Usa el callback ya registrado
    ...
    # Obtener buffer y servicio en depósito
    buffer_min = getattr(request, 'buffer_minutes', 0)
    depot_idx = request.depot
    service_time_depot = getattr(request.locations[depot_idx], 'service_time', 0)
    # Activar ajuste de ventana de salida en depósito por defecto
    adjust_depot_window = getattr(request, 'adjust_depot_window', True)
        ...
        if adjust_depot_window:
            ...
            for idx, tw in enumerate(time_windows):
                if idx == request.depot:
                    continue
                # No puede salir antes del horario laboral, ni antes de lo necesario para llegar justo a tiempo (considerando servicio, buffer y viaje)
                travel_time = time_matrix[request.depot][idx]
                candidate_departure = max(
                    start_time_orig,
                    tw[0] - travel_time - service_time_depot - buffer_min
                )
    ...
    search_parameters = get_advanced_search_parameters(n)
