...
class VRPConstants:
    ...
    """
    # Jornada laboral en minutos (inmutables)
    ...
    MAX_LOCATIONS = 100   # Máximo de ubicaciones permitidas
    MAX_VEHICLES = 10     # Máximo de vehículos permitidos
...
def add_warning(warnings_list, code, message, context=None):
    ...
    Args:
        warnings_list: Lista de advertencias (puede ser None)
    ...
    logger.warning(f"[WARNING {code}] {message}")
    if context:
...
def validate_request_coords(request) -> None:
    """
    Validaciones esenciales de ubicaciones:
    - Mínimo 2 ubicaciones (origen y destino)
    - Todas deben tener coordenadas válidas
    """
    if len(request.locations) < 2:
        raise HTTPException(
            status_code=400,
            detail="Se requieren al menos 2 ubicaciones (origen y destino)"
        )
        
    for i, loc in enumerate(request.locations):
        if not hasattr(loc, 'lat') or not hasattr(loc, 'lon'):
            name = getattr(loc, 'name', f'#{i}')
            raise HTTPException(
                status_code=400,
                detail=f"La ubicación {name} no tiene coordenadas válidas (lat, lon)"
            )
